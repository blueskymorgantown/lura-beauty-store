// ============================================================
// LURA Beauty — app.js — منطق التطبيق الكامل
// ============================================================
...
function renderStore(){
  ...
  var list=products.filter(function(p){
    var matchCat=currentCatFilter==='all'||p.cat===currentCatFilter;
    var matchQ=!q||p.name.toLowerCase().includes(q)||p.brand.toLowerCase().includes(q)||(p.nameAr&&p.nameAr.includes(q));
  ...
  if(!list.length){
    grid.innerHTML='<div style="grid-column:1/-1;padding:4rem;text-align:center;color:var(--mid);font-family:Cormorant Garamond,serif;font-style:italic;font-size:22px;background:#fff">لا توجد منتجات مطابقة</div>';
    return;
  }
  grid.innerHTML=list.map(productCardHTML).join('');
}
function productCardHTML(p){
  var img=p.img?'<img src="'+p.img+'" alt="'+p.name+'">'
    :'<i class="fa fa-leaf placeholder-icon"></i>';
  return '<div class="product-card">'
    ...
    +'<div class="product-price">'+p.price+' <small>SR</small></div>'
...
function updateCartUI(){
  ...
  var sub=cart.reduce(function(s,i){return s+i.price*i.cartQty},0);
  var vat=sub*0.15;var total=sub+vat;
  document.getElementById('cart-sub').textContent=sub.toFixed(2)+' SR';
  document.getElementById('cart-vat').textContent=vat.toFixed(2)+' SR';
  document.getElementById('cart-total').textContent=total.toFixed(2)+' SR';
  ...
  ci.innerHTML=cart.map(function(item){
    ...
    return '<div class="cart-item">'
      ...
      +'<div class="cart-item-price">'+(item.price*item.cartQty).toFixed(0)+' SR</div>'
...
function checkout(){
  ...
  var lines=cart.map(function(i){return '• '+i.brand+' — '+i.name+' × '+i.cartQty+' = '+(i.price*i.cartQty).toFixed(0)+' SR'});
  var sub=cart.reduce(function(s,i){return s+i.price*i.cartQty},0);
  var msg='طلب جديد — LURA Beauty Collection\n\n'+lines.join('\n')+'\n\nالمجموع: '+(sub*1.15).toFixed(2)+' SR (شامل ضريبة 15%)';
...
function renderAdmin(){
  ...
  document.getElementById('admin-stats').innerHTML=
    ...
    +'<div class="admin-stat"><div class="admin-stat-num">'+total.toFixed(0)+'</div><div class="admin-stat-label">قيمة المخزون SR</div></div>'
    +'<div class="admin-stat"><div class="admin-stat-num">'+products.filter(function(p){return p.qty===0}).length+'</div><div class="admin-stat-label">نفذت الكمية</div></div>';
  ...
  document.getElementById('admin-tbody').innerHTML=list.map(function(p){
    var qtyColor=p.qty===0?'color:var(--red)':p.qty<3?'color:orange':'';
    return '<tr>'
      +'<td style="color:var(--mid);font-size:10px">'+p.id+'</td>'
      +'<td style="font-size:10px;letter-spacing:1px;color:var(--rose)">'+p.brand+'</td>'
      +'<td><div style="font-size:12px;font-weight:500">'+p.name+'</div><div style="font-size:10px;color:var(--mid)">'+p.nameAr+'</div></td>'
      +'<td><span class="cat-badge '+catBadge[p.cat]+'" style="position:static;display:inline-block">'+catLabels[p.cat]+'</span></td>'
      +'<td style="font-weight:500;'+qtyColor+'">'+p.qty+'</td>'
      +'<td style="font-family:Cormorant Garamond,serif;font-size:17px">'+p.price+' SR</td>'
      +'<td style="font-family:Cormorant Garamond,serif;font-size:15px;color:var(--accent)">'+(p.price*p.qty).toFixed(0)+' SR</td>'
      +'<td><div class="action-btns">'
      +'<button class="btn-edit" onclick="openModal('+p.id+')"><i class="fa fa-edit"></i> تعديل</button>'
...
function saveProduct(){
  ...
  var img=(imgEl&&imgEl.style.display!=='none'&&imgEl.src)?imgEl.src:'';
...
function renderPicker(){
  ...
  g.innerHTML=list.map(function(p){
    return '<div class="picker-item'+(selectedProductId===p.id?' selected':'')+'" onclick="selectedProductId='+p.id+';renderPicker()">'
      ...
      +'<div class="picker-item-name">'+(p.name.length>45?p.name.substring(0,45)+'…':p.name)+'</div>'
      +'</div>';
  }).join('');
}
function generateDesign(){
  ...
  if(currentTemplate==='elegant'){
    ...
    ctx.font='300 '+(18*s)+'px sans-serif';ctx.globalAlpha=0.6;
    ctx.fillText('BEAUTY COLLECTION',canvasW/2,canvasH*0.275);ctx.globalAlpha=1;
    ...
    ctx.fillText(p.price+' SR',canvasW/2,canvasH*0.80);
    ctx.font='300 '+(16*s)+'px sans-serif';ctx.fillStyle=ac;ctx.globalAlpha=0.6;
    ctx.fillText('الكمية المتاحة: '+p.qty,canvasW/2,canvasH*0.865);
    if(handle){ctx.font='300 '+(16*s)+'px sans-serif';ctx.fillStyle=ac;ctx.globalAlpha=0.5;ctx.fillText(handle,canvasW/2,canvasH*0.93);}
    ctx.globalAlpha=1;
    ...
    ctx.font='300 '+(76*s)+'px serif';ctx.fillStyle=ac;
    ctx.fillText(p.price+' SR',canvasW/2,canvasH*0.83);
    if(handle){ctx.font='300 '+(15*s)+'px sans-serif';ctx.fillStyle=tc;ctx.globalAlpha=0.4;ctx.fillText(handle,canvasW/2,canvasH*0.92);}
    ctx.globalAlpha=1;

  } else if(currentTemplate==='minimal'){
    ctx.strokeStyle=ac;ctx.lineWidth=0.5*s;ctx.globalAlpha=0.2;
    ctx.beginPath();ctx.moveTo(80*s,canvasH/2);ctx.lineTo(canvasW-80*s,canvasH/2);ctx.stroke();
    ctx.globalAlpha=1;
    ctx.font='200 '+(22*s)+'px sans-serif';ctx.fillStyle=tc;ctx.globalAlpha=0.45;ctx.textAlign='center';
    ...
    ctx.fillText(p.price+' SR',canvasW/2,canvasH*0.78);
    ctx.font='300 '+(18*s)+'px serif';ctx.fillStyle=ac;ctx.globalAlpha=0.7;
    ctx.fillText(p.nameAr,canvasW/2,canvasH*0.86);ctx.globalAlpha=1;
    if(handle){ctx.font='200 '+(14*s)+'px sans-serif';ctx.fillStyle=tc;ctx.globalAlpha=0.3;ctx.fillText(handle,canvasW/2,canvasH*0.94);}
    ctx.globalAlpha=1;

  } else if(currentTemplate==='sale'){
    ctx.fillStyle=ac;ctx.fillRect(0,0,canvasW,90*s);ctx.fillRect(0,canvasH-90*s,canvasW,90*s);
    ctx.font='400 '+(20*s)+'px sans-serif';ctx.fillStyle=isLight?tc:'#fff';ctx.textAlign='center';
    ctx.fillText('✦  LURA BEAUTY COLLECTION  ✦',canvasW/2,58*s);
    // badge
    ctx.save();ctx.fillStyle=isLight?'#3d2b47':ac;
    ctx.beginPath();ctx.arc(canvasW*0.78,canvasH*0.24,95*s,0,Math.PI*2);ctx.fill();ctx.restore();
    ctx.font='500 '+(extraText?24*s:22*s)+'px sans-serif';ctx.fillStyle='#fff';
    ctx.fillText(extraText||'SALE',canvasW*0.78,canvasH*0.24+9*s);
    ctx.font='400 '+(24*s)+'px sans-serif';ctx.fillStyle=ac;
    ctx.fillText(p.brand,canvasW/2,canvasH*0.38);
    ctx.font='300 '+(34*s)+'px serif';ctx.fillStyle=tc;
    ...
    ctx.font='300 '+(82*s)+'px serif';ctx.fillStyle=tc;
    ctx.fillText(p.price+' SR',canvasW/2,canvasH*0.82);
    if(handle){ctx.font='300 '+(17*s)+'px sans-serif';ctx.fillStyle='#fff';ctx.globalAlpha=0.8;ctx.fillText(handle,canvasW/2,canvasH-40*s);}
    ctx.globalAlpha=1;

  } else if(currentTemplate==='luxury'){
    // gold pattern
    ctx.strokeStyle=ac;ctx.lineWidth=0.5*s;ctx.globalAlpha=0.15;
    for(var i=0;i<10;i++){ctx.beginPath();ctx.arc(canvasW/2,canvasH/2,(80+i*60)*s,0,Math.PI*2);ctx.stroke();}
    ctx.globalAlpha=1;
    ctx.font='200 '+(16*s)+'px sans-serif';ctx.fillStyle=ac;ctx.textAlign='center';ctx.globalAlpha=0.5;
    ctx.fillText('✦  ✦  ✦',canvasW/2,canvasH*0.18);ctx.globalAlpha=1;
    ctx.font='200 '+(100*s)+'px serif';ctx.fillStyle=ac;
    ctx.fillText('LURA',canvasW/2,canvasH*0.32);
    ctx.font='200 '+(16*s)+'px sans-serif';ctx.globalAlpha=0.4;
    ctx.fillText('— BEAUTY COLLECTION —',canvasW/2,canvasH*0.38);ctx.globalAlpha=1;
    ...
    ctx.font='300 '+(70*s)+'px serif';ctx.fillStyle=tc;
    ctx.fillText(p.price+' SR',canvasW/2,canvasH*0.82);
    if(handle){ctx.font='200 '+(14*s)+'px sans-serif';ctx.fillStyle=ac;ctx.globalAlpha=0.4;ctx.fillText(handle,canvasW/2,canvasH*0.92);}
    ctx.globalAlpha=1;
  }
  ...
  if(p.img){
    ...
    imgObj.onload=function(){
      var imgS=140*s;
      ctx.drawImage(imgObj,canvasW*0.06,canvasH*0.05,imgS,imgS);
    };
    imgObj.src=p.img;
  }

  document.getElementById('canvas-info').textContent=canvasW+' × '+canvasH+' px — جاهز للتحميل';
...
function downloadDesign(fmt){
  var canvas=document.getElementById('designCanvas');
  if(canvas.width===0||canvas.width<100){showNotif('أنشئ التصميم أولاً','error');return;}
  var a=document.createElement('a');
  a.download='lura-design-'+Date.now()+'.'+fmt;
  a.href=canvas.toDataURL(fmt==='jpg'?'image/jpeg':'image/png',0.96);
  a.click();
  showNotif('✓ تم التحميل بنجاح');
}
